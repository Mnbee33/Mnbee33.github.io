...
プロパーレビュー（一次請会社の社員による設計書レビュー）の指摘が多かった。設計書の作成ルール違反による指摘が半数以上を占め、レビューアからも成果物の品質に関して苦言を呈される程であった。なぜ設計者はルール違反をしてしまうのか、チーム内のレビューアは違反を見つけられないのか、大きく二つの原因に当たりをつけた。
...
- チェックリストを作成
  - ルールの総数が、設計者もチーム内のレビューアも記憶できる範囲を超えていたため、作成ルールを一覧化した。設計者には取り込み漏れがないかのセルフチェックシートとして、チーム内のレビューアにはレビューの観点表として確認してもらった。
