...
- 電力の配電工事について、設計から工事が完了するまでの工事状況をWeb画面で管理するシステム
- チームが担当した領域は、
  - 工事を担当する会社への依頼、発注状況の管理機能
  - 工事の工数管理
- チーム規模：最大12名
...
　プロパーレビュー（一次請会社の社員による設計書レビュー）の指摘が多かった。指摘の半数以上は、プロジェクトの作成ルールを違反していたことによるものであった。他のチームよりも指摘が多く、レビューアからも、成果物の品質に関して苦言を呈された。

1. 元々、ルールの数が多い。

   １機能につき、10種類以上のドキュメントが必要。１ドキュメントあたりのルール数も片手では収まらない。

2. 設計書の作成前に、顧客・プロパーから提示されたルールが絶対ではない。

   元請け会社レビュー・プロパーレビュー（他の機能の分も含む）の指摘から、設計書の新たな改善点が発見される。レビューを重ねるごとに、ルールが増える。  

　状況を改善するため、いくつかの対策を考え、実行した。  

- 設計書のチェックツールを自作
  - 設計書の作成ルールが膨大なことへの対策。ルールブック・サンプルを見ながらの作成、確認が必須ではあるが、目視ではどうしても見落としが発生する。
  - ルールどおり作成できているか、ツールでまとめてチェックできるようにした。目視では検知が不十分だったポイントも、漏れなくチェックできるようになった。チェックにかかる時間も20分から1〜2分まで短縮。
- ルール追加・変更の周知
  - 朝一番に（他チームの成果物も含めて）レビュー記録票を確認。設計書作成のルールの追加や変更があった場合、朝会とメールでチーム全体に周知した。
- チェックリストを作成
  - 追加されたルールだけで30個超え、ということも。設計者もチーム内のレビューアも覚えきれる数ではなくなっていたため、作成ルールを一覧化した。設計者には取り込み漏れがないかのセルフチェックとして、チーム内のレビューアにはレビューの観点表として確認してもらった。
...
​	参画直後、単体試験で類似した不具合が頻発した。チーム内にライブラリの有識者がいなかったため、上長と二人で調査にあたった。調査と試行錯誤の末、原因を突き止めた。その後、単体試験のマニュアルを作成し、チーム内に展開。メンバに対面でも注意喚起や指導を行った。その結果、単体試験の不具合は解消された。
...
> 参画時、顧客から指定されたフレームワーク・ライブラリの使用経験のあるチームメンバは、自分を含めていなかった。円滑にプロジェクトを進めるためには、少しでも早く、新しい技術に馴染むことが不可欠だった。
>
> 1. 新しい技術の学習
...
>    - 既存ソースコードの解析結果や、ガイドラインの和訳など、自学自習したことをドキュメントにまとめた。ドキュメントの作成過程で、自分自身の考え・知識も整理。作成したドキュメントは他メンバの理解の手助けとなるよう、チーム内に展開した。
